...
export default function HistoryPage() {
  ...
  const fetchAppointments = async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`${(typeof process !== 'undefined' && process.env.NEXT_PUBLIC_API_URL ? process.env.NEXT_PUBLIC_API_URL : '/api')}/appointments`, { credentials: 'include' });
      if (res.status === 401) {
  ...
  const handleCancel = async (appointmentId: string) => {
    ...
    try {
      const res = await fetch(`${(typeof process !== 'undefined' && process.env.NEXT_PUBLIC_API_URL ? process.env.NEXT_PUBLIC_API_URL : '/api')}/appointments/${appointmentId}/cancel`, {
        method: 'PUT',
        credentials: 'include'
      });
      const data = await res.json();
